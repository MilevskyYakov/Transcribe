...
from transcribe_doc.asr.model_registry import DEFAULT_WHISPER_MODELS, EXTERNAL_MODELS, external_spec
...
def inspect_whisper_models(model_names: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    from transcribe_doc.asr.external_model_cache import inspect_external_model
...
def ensure_whisper_model_ready(model_name: str) -> None:
    ...
    if external_spec(model_name) is not None:
        from transcribe_doc.asr.external_model_cache import ensure_external_model_ready
...
def download_whisper_model(model_name: str, progress_callback: ModelProgress | None = None) -> dict[str, Any]:
    ...
    if external is not None:
        from transcribe_doc.asr.external_model_cache import download_external_model
